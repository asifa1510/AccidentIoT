/*
  ESP32 REAL-TIME ACCIDENT DETECTION (NO NETWORK)
  - MPU6050  I2C: SDA=21, SCL=22
  - NEO-6M   UART2: RX2=16 (GPS TX), TX2=17 (GPS RX)
  - Piezo seat sensor: GPIO34 with bias + clamp
  - Buzzer: GPIO18
  - MOSFET gate: GPIO23
*/
...
#include "driver/adc.h"   // Needed for analogSetPinAttenuation()
...
const int GPS_RX = 16; // ESP32 RX
const int GPS_TX = 17; // ESP32 TX
const uint32_t GPS_BAUD = 9600;

// Seat detector
const int PIEZO_ADC_PIN = 34;
const adc_attenuation_t PIEZO_ATTEN = ADC_11db; // fixed type
...
const uint32_t IMPACT_WINDOW_MS = 120;
const uint32_t LATCH_HOLDOFF_MS = 10000;

// Severity thresholds
const float A_MINOR_G = 2.0f,  J_MINOR = 5.0f,  W_MINOR = 250.0f;
const float A_MOD_G   = 3.0f,  J_MOD   = 8.0f,  W_MOD   = 300.0f;
const float A_SEV_G   = 5.0f,  J_SEV   = 15.0f, W_SEV   = 500.0f;
// ----------------------------
...
inline float lpf(float prev, float input, float alpha) {
...
void emitAccident(const String& sev, float a, float j, float w) {
  Serial.printf("ACCIDENT,%s,%.2f,%.1f,%.0f,%.1f,%.6f,%.6f\n",
                sev.c_str(), a, j, w, lastSpeedKmph, lastLat, lastLng);
  buzz(600);
}
...
void loop() {
  ...
  if ((nowMs - lastTriggerMs) > 15000) setFuel(true);

  if (nowMs - lastDbg > 250) {
    lastDbg = nowMs;
    Serial.printf("a=%.2fg jerk=%5.1fg/s w=%3.0fdps seat=%c seatE=%.1f gps=%c\n",
